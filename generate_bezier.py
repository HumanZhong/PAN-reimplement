...
labels = glob.glob('/home/data1/IC19/CTW1500/train/text_label_curve/*.txt')
labels.sort()
...
for il, label in tqdm(enumerate(labels)):
    ...
    imgdir = label.replace('/home/data1/IC19/CTW1500/train/text_label_curve/', '/home/data1/IC19/CTW1500/train/text_image/').replace('.txt', '.jpg')
    data = []
    cts = []
    polys = []
    ...
    for il, line in enumerate(fin):
        ...
        # if ct == '#': continue
        # print('ct', ct)
        coords = [(float(int(line[ix]) + x0), float(int(line[ix + 1]) + y0)) for ix in range(4, len(line), 2)]
        poly = Polygon(coords)
        line = line[4:]
