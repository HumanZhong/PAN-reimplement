...
def train(model, trainloader, criterion, optimizer, epoch):
    ...
    for batch_idx, (imgs, gt_texts, gt_kernels, gt_tops, gt_bots, training_masks, i_channels, j_channels) in enumerate(trainloader):
        ...
        outputs = model(imgs, i_channels, j_channels)
        ...
        mask2 = gt_tops.data.cpu().numpy()
        mask3 = gt_texts.data.cpu().numpy()
        mask_top = torch.sigmoid(output_tops).data.cpu().numpy()
        selected_top_masks = ((mask1 > 0.5) & ((mask2 > 0.5) | (mask3 > 0.5))).astype('float32')
        ...
        mask1 = training_masks.data.cpu().numpy()
        mask2 = gt_bots.data.cpu().numpy()
        mask3 = gt_texts.data.cpu().numpy()
        mask_bot = torch.sigmoid(output_bots).data.cpu().numpy()
        selected_bot_masks = ((mask1 > 0.5) & ((mask2 > 0.5) | (mask3 > 0.5))).astype('float32')
        ...
        loss = 1.0 * loss_text + 0.2 * loss_kernel + 0.5 * (loss_top + loss_bot)
...
def main(args):
    ...
    trainset = CTW1500Trainset_Bound(with_coord=True)
    ...
    for epoch in range(n_epoch):
        ...
        torch.save(checkpoint_info, '/home/data1/zhm/ctw_bound_checkpoint_newwidth_coordconv_0330_200e.pth.tar')

if __name__ == '__main__':
    parser = argparse.ArgumentParser()
    parser.add_argument('--backbone', nargs='?', type=str, default='res50')
    parser.add_argument('--schedule', nargs='+', type=int, default=[150, 250])
