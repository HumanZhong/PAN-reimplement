...
class ResNet(nn.Module):
    ...
    def __init__(self, block, layers, num_classes=7, scale=1):
        ...
        # TODO: implement the CoordConv to see if it works better than standard Conv
        # i_channel = np.zeros(shape=(160, 160)).astype(float)
        # j_channel = np.zeros(shape=(160, 160)).astype(float)
        # for i in range(160):
        #     i_channel[i, :] = -1 + i / 80
        #     j_channel[:, i] = -1 + i / 80
        # i_channel = np.expand_dims(i_channel, axis=0)
        # i_channel = np.expand_dims(i_channel, axis=0).repeat(1, axis=0)
        # i_channel = torch.from_numpy(i_channel).float()
        # self.i_channel = Variable(i_channel.cuda())
        #
        # j_channel = np.expand_dims(j_channel, axis=0)
        # j_channel = np.expand_dims(j_channel, axis=0).repeat(1, axis=0)
        # j_channel = torch.from_numpy(j_channel).float()
        # self.j_channel = Variable(j_channel.cuda())



        self.coord_conv = nn.Conv2d(1026, 256, kernel_size=3, stride=1, padding=1)
    ...
    def forward(self, x, i_channels=None, j_channels=None):
        h = x
        h = self.conv1(h)
        h = self.bn1(h)
        h = self.relu1(h)
        ...
        #     j_channel[:, j] = -1 + j / w * 2
        # i_channel = np.expand_dims(i_channel, axis=0)
        # i_channel = np.expand_dims(i_channel, axis=0).repeat(batch_size, axis=0)
        # i_channel = torch.from_numpy(i_channel).float()
        # i_channel = Variable(i_channel.cuda())
        #
        # j_channel = np.expand_dims(j_channel, axis=0)
        # j_channel = np.expand_dims(j_channel, axis=0).repeat(batch_size, axis=0)
        # j_channel = torch.from_numpy(j_channel).float()
        # j_channel = Variable(j_channel.cuda())

        if i_channels is not None and j_channels is not None:
            out = torch.cat((p2, p3, p4, p5, i_channels, j_channels), 1)
